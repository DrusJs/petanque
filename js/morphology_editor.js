const Editor = ( { domContainer, json, backgroundURL, fontList } ) =>
{
	...
	const stage = paper.g().attr( { id:'stage' } );
	const root = stage.g().attr( { id:'root' } );
	...
	const hoverBounds = stage.g().attr( { id:'hover-bounds' } ).attr
	( {
		stroke:'#000',
		strokeWidth:1,
	...
	const selectedBounds = stage.g().attr( { id:'selected-bounds' } ).attr
	( { 
		stroke:'black', 
		strokeWidth:1, 
		fill:'none', 
	} );
	const handles = stage.g().attr( { id:'handles' } );
	...
	const updateStage = () =>
	{
		...
		paper.attr( { width:rect.width, height:rect.height } );
		
		stage.transform( new Snap.Matrix().translate( rect.width / 2, rect.height / 2 ) );
	};
	...
	const getElementAttributes = ( uuid ) => 
	{
		...
		if( element )
		{
			...
			{								
				...
				attrs.strokeColor = Snap.getRGB( element.data().strokeColor ).hex;
				attrs.strokeWidth = element.data().strokeWidth;
			}
	...
	const setAttributes = ( element, attrs, forceUpdate = false ) => 
	{
		...
		if( attrs != null && attrs.hasOwnProperty( 'strokeColor' ) )
		{
			element.data().strokeColor = attrs.strokeColor;
			
			const feFlood = element.data().shadowFilter.select( 'feFlood[result=outside_color]' );
			
			feFlood.attr( { floodColor:element.data().strokeColor } );

		}
		
		if( attrs != null && attrs.hasOwnProperty( 'strokeWidth' ) )
		{
			element.data().strokeWidth = attrs.strokeWidth;

			const feMerge = element.data().shadowFilter.select( 'feMerge' );
			const feMergeNode = feMerge.select( 'feMergeNode[in=outside_stroke]' );
			const feMorphology = element.data().shadowFilter.select( 'feMorphology' );
			
			feMorphology.attr( { radius:element.data().strokeWidth } );
			
			if( element.data().strokeWidth <= 0 && feMergeNode )
				feMergeNode.remove();
			
			if( element.data().strokeWidth > 0 && !feMergeNode )
				feMerge.append( Snap.parse( '<feMergeNode in="outside_stroke"/>' ) );	
		}
	...
	const createShadowFilter = ( element ) =>
	{
		...
		element.data().shadowFilter.append
		( Snap.parse( 
			'<feFlood flood-color="' + element.data().strokeColor + '" result="outside_color"/>' +
			'<feMorphology in="SourceAlpha" operator="dilate" radius="' + element.data().strokeWidth + '"/>' + 
			'<feComposite in="outside_color" operator="in" result="outside_stroke"/>' 
		) );
		element.data().shadowFilter.append( Snap.parse( '<feMerge />' ) );
		
		if( element.data().strokeWidth > 0 )
		{
			const feMerge = element.data().shadowFilter.select( 'feMerge' );
	...
	{						
		...
		element.data().graphics = element.g().attr
		( { 
			/*visibility:'hidden',*/ 
			filter:element.data().shadowFilter
		} );
	...
	const translate = ( sx, sy, ex, ey ) =>
	{
		const localStartPoint = globalToLocal( stage, sx, sy );
		const localEndPoint = globalToLocal( stage, ex, ey );
		const localMatrix = root.transform().localMatrix;
		
		root.transform( new Snap.Matrix().translate( localEndPoint.x - localStartPoint.x, localEndPoint.y - localStartPoint.y ).add( localMatrix ) );
		...
		{
			drawHoverBounds( hoverElement );
		}		
	};
	
	const zoom = ( scale, cx, cy ) => 
	{
		const localPoint = globalToLocal( stage, cx, cy );
		const localMatrix = root.transform().localMatrix;
		const matrix = new Snap.Matrix().scale( scale, scale, localPoint.x, localPoint.y ).add( localMatrix );
		const transform = matrix.split();
		
		if( transform.scalex > 0.25 && transform.scalex < 8.0 )
	...
	const resetView = () =>
	{
		root.transform( new Snap.Matrix().translate( -width / 2, -height / 2 ) );	
	...
	{
		const bbox = element.getBBox( true );	
		const scale = element.transform().globalMatrix.split().scalex;
		const rotation = element.transform().globalMatrix.split().rotate;
		const offset = HANDLE_SIZE / scale;
		
		if( rotationHandle )
		{
			const point = localToLocal( element, handles, bbox.x2 + offset * 0.5, bbox.y - offset * 1.5 );
			rotationHandle.transform( new Snap.Matrix().translate( point.x, point.y ).rotate( rotation ) );
		}
		
		if( widthHandle )
		{
			const point = localToLocal( element, handles, bbox.x2 + offset * 0.5, bbox.y - offset * 0.5 + bbox.height / 2 );
			widthHandle.transform( new Snap.Matrix().translate( point.x, point.y ).rotate( rotation ) );
		}
		
		if( widthHandle2 )
		{
			const point = localToLocal( element, handles, bbox.x - offset * 1.5, bbox.y - offset * 0.5 + bbox.height / 2 );
			widthHandle2.transform( new Snap.Matrix().translate( point.x, point.y ).rotate( rotation ) );
		}
		
		if( radiusHandle )
		{
			const point = localToLocal( element, handles, bbox.x2 + offset * 0.5, bbox.y2 + offset * 0.5 );
			radiusHandle.transform( new Snap.Matrix().translate( point.x, point.y ).rotate( rotation ) );
		}
		
		if( arcLengthHandle )
		{
			const point = localToLocal( element, handles, element.data().pathEndX - offset * 0.5, element.data().pathEndY - offset * 0.5 );
			arcLengthHandle.transform( new Snap.Matrix().translate( point.x, point.y ).rotate( rotation ) );
		}
	};
	...
	const clearHoverBounds = () => hoverBounds.clear();
	const drawHoverBounds = ( element ) => 
	{
		...
		const points = 
		[
			{ x:bbox.x - offset, y:bbox.y - offset },
			{ x:bbox.x + bbox.width + offset, y:bbox.y - offset },
			{ x:bbox.x + bbox.width + offset, y:bbox.y + bbox.height + offset },
			{ x:bbox.x - offset, y:bbox.y + bbox.height + offset },
		];
		
		points.forEach( ( point, index ) => points[ index ] = localToLocal( element, hoverBounds, point.x, point.y ) );
		...
		for( let i = 0; i < length; i++ )
		{
			...
			hoverBounds.line( point1.x, point1.y, point2.x, point2.y );
		}
	};
	...
	const drawSelectedBounds = ( element ) => 
	{
		...
		points.forEach( ( point, index ) => points[ index ] = localToLocal( element, selectedBounds, point.x, point.y ) );
		...
		for( let i = 0; i < length; i++ )
		{
			...
			selectedBounds.line( point1.x, point1.y, point2.x, point2.y );
		}
	};
	...
	const setPointerDown = ( value, button = 0 ) => 
	{	
		...
		if( !isPointerDown )
		{
			/*if( button == 0  ) 
			{
				...
				( 
					selectedElement != null && 
					pointerX == pointerDownX && 
					pointerY == pointerDownY && 
					!isPointInside( selectedElement, pointerX, pointerY ) && 
					getHandleAtPoint( pointerX, pointerY ) == null )
				{
					// deselect
					removeSelection();
				}	
	...
	const setPointer = ( x, y ) => 
	{
		...
		if( hoverEnabled && action != 'moveScene' ) 
		{	
			const topBBoxElement = getElementAtPoint( pointerX, pointerY, false );
			const topHitTestedElement = getElementAtPoint( pointerX, pointerY, true );
			const topElement = topHitTestedElement || topBBoxElement;
		...
		if( action == 'moveScene' )
		{
			translate( prevPointerX, prevPointerY, pointerX, pointerY );
		}
	...
	{
		if( !( event.isPrimary && ( event.button == 0 || event.button == 2 ) ) )
			return;
		...
		setPointerDown( true, event.button );
	} );
	...
	window.addEventListener( 'pointerup', event => 
	{
		...
		setPointer( event.clientX - rect.left, event.clientY - rect.top );	
		setPointerDown( false, event.button );	
	} );
	
	paperElement.addEventListener( 'wheel', event => 
	{
		...
		if( event.deltaY < 0 )
			zoom( 1 / 0.95, pointerX, pointerY );
		else 
			zoom( 1 * 0.95, pointerX, pointerY );
	} );
	
	window.addEventListener( 'resize', event => updateStage() );
