const Viewer = ( { domContainer, json } ) =>
{
	...
	return {
		...
		toDataURL:( callback ) => 
		{
			...
			const emptyLength  = canvas.toDataURL().length;
			...
			{
				context.clearRect( 0, 0, width, height );
				context.drawImage( paperImage, 0, 0 );
				
				const dataURL = canvas.toDataURL();
				const currentLength = dataURL.length;
				
				// console.log( 'toDataURL.onTimer[ ' + emptyLength + ', ' + currentLength + ' ]' );
			...
			paperImage.src = paper.toDataURL();
